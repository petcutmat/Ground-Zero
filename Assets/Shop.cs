...
public class Shop : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Item i1 = new Item(flowerImage, "Loto", 1500);
        itemList.Add(i1);
        Item i2 = new Item(floorImage, "Teleregreso", 750);
        itemList.Add(i2);
        Item i3 = new Item(smokeImage, "Bomba de humo", 200);
        itemList.Add(i3);
        Reshuffle();
    }
    ...
    public void BuyItem(GameObject item)
    {
        ...
        if (invItems < 2){
            ...
            if (master.GetComponent<MasterScript>().players.transform.GetChild(master.GetComponent<MasterScript>().whosTurn - 1).GetComponent<Points>().points >= int.Parse(srPoints)) { 
                master.GetComponent<MasterScript>().players.transform.GetChild(master.GetComponent<MasterScript>().whosTurn - 1).GetComponent<Points>().points -= int.Parse(srPoints); //pagar objeto
                foreach (GameObject inv in inventory){
                    if (inv.transform.GetChild(0).GetComponent<Image>().sprite == placeHolderSprite)
                    {
                        GetComponent<AudioSource>().Play();
                        inv.transform.GetChild(0).GetComponent<Image>().sprite = item.transform.GetChild(1).GetChild(0).GetComponent<Image>().sprite;
                        if (item.transform.GetChild(1).GetChild(0).GetComponent<Image>().sprite == flowerImage.GetComponent<SpriteRenderer>().sprite) inv.GetComponent<Button>().onClick.AddListener(() => ActivateLotus(inv) );
                        if (item.transform.GetChild(1).GetChild(0).GetComponent<Image>().sprite == floorImage.GetComponent<SpriteRenderer>().sprite) inv.GetComponent<Button>().onClick.AddListener(() => ActivateTpFloor(inv) );
                        if (item.transform.GetChild(1).GetChild(0).GetComponent<Image>().sprite == smokeImage.GetComponent<SpriteRenderer>().sprite) inv.GetComponent<Button>().onClick.AddListener(() => ActivateSmoke(inv) );
                    break;
                    }
                }
                Reshuffle();
    ...
    void ActivateTpFloor(GameObject inv)
    {
        ...
        inv.transform.GetChild(0).GetComponent<Image>().sprite = placeHolderSprite;
        StartCoroutine(master.GetComponent<MasterScript>().players.transform.GetChild(randPlayer).GetComponent<Movement>().TpBack(10,randPlayer+1));
        inv.GetComponent<Button>().onClick.RemoveAllListeners();
